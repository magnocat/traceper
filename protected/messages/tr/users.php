<?php 
/**
 * This file is the translation of files under views/users folder.
 */

/*
 * Use the below corresponding Utf-8 characters, if you have problems with Turkish characters
T�rk�e UTF-8
� 	İ
� 	ı
� 	Ö
� 	ö
� 	Ü
� 	ü
� 	Ç
� 	ç
� 	ğ
� 	ğ
� 	�
� 	ş

�		&#304;
�		&#305;
�		&#214;	
�		&#246;
�		&#220;
�		&#252;
�		&#199;
�		&#231;
�		&#286;
�		&#287;
�		&#350;
�		&#351;
*/
return array(
	'No users found registered by this name unfortunately. You should invite your friend to Traceper first by the link {inviteIcon}(Invite Friends) at the top menu or by {inviteByHere}. After he/she joins Traceper, you could be friends.' => 'Maalesef bu isme uygun kayıtlı kullanıcı bulunamadı. Arkadaşınızı üst menüdeki {inviteIcon}(Arkadaşlarını Davet Et) bağlantısından veya {inviteByHere} öncelikle Traceper\'a üye olmaya davet etmelisiniz. Kendisi Traceper\'a katıldıktan sonra arkadaş olabilirsiniz.',
	'You do not have any friend at the moment. In order to add new friends, you could search them by name and choose from the list. If you could not find your friends by search, you could invite them to Traceper first by the link {inviteIcon}(Invite Friends) at the top menu or by {inviteByHere}.' => 'Şu an için hiç arkadaşınız yok. Yeni arkadaşlar eklemek için, yukarıdaki arama alanına isimlerini yazarak arayabilir ve çıkan listeden seçim yaparak ekleyebilirsiniz. Arama sonucunda arkadaşlarınızı bulamadığınız takdirde, üst menüdeki {inviteIcon}(Arkadaşlarını Davet Et) bağlantısından veya {inviteByHere} öncelikle onları Traceper\'a davet edebilirsiniz.',		
	'Do you really want to delete this user from your friend list?' => 'Bu kullanıcıyı arkadaş listenizden gerçekten çıkarmak istiyor musunuz?',
	'Do you want to add this user as a friend?' => 'Bu kullanıcıyı arkadaş olarak eklemek istiyor musunuz?',	
	'Do you want to reject this user\'s friend request?' => 'Bu kullanıcının arkadaşlık isteğini reddetmek istiyor musunuz?',
	'No friendship requests found' => 'Arkadaşlık isteği bulunamadı',
	'Friend request is sent...' => 'Arkadaşlık isteği gönderildi...',
	'Group Settings' => 'Grup Ayarları',
	'Geofence Settings' => 'Jeoçit Ayarları',
	'No users to show...' => 'Kullanıcı bilgileri mevcut değil...',
	'Approve' => 'Onayla',
	'Reject' => 'Reddet',
	'Delete from your friend list' => 'Arkadaş listenden sil',	
	'Do you really want to delete the account of this staff?' => 'Bu personelin hesabını gerçekten silmek istiyor musunuz?',
	'Waiting reply for your friendship request' => 'Arkadaşlık isteğinize yanıt bekleniyor',
	'Add as Friend' => 'Arkadaş olarak ekleyin',
	'Friendship Requests' => 'Arkadaşlık İstekleri',
	'The friendship request is approved, you are now friends...' => 'Arkada&#351;l&#305;k iste&#287;i onayland&#305;, art&#305;k arkada&#351;s&#305;n&#305;z...',
	'You have rejected the friendship request' => 'Arkada&#351;l&#305;k iste&#287;ini geri çevirdiniz',
	'The person is removed from your friend list' => 'Ki&#351;i arkada&#351; listenizden &#231;&#305;kar&#305;ld&#305;',
	'Already your friend' => 'Zaten arkadaşınız',
	'This user wants to be friend with you. You can approve or reject the request via <Friendship Requests> menu at the top' => 'Bu kişi sizinle arkadaş olmak istiyor. Üst bölümdeki <Arkadaşlık İstekleri> menüsünü kullanarak isteği kabul ya da reddedebilirsiniz',
	'See your friend\'s position on the map' => 'Arkadaşınızın konumunu haritada görün',
	'wants to be your friend at Traceper' => 'Traceper\'da sizinle arkadaş olmak istiyor',
	'If you wish you could accept or reject this friendship request using the "Friendship Requests" menu of your mobile application or at address www.traceper.com.' => 'Dilerseniz mobil uygulamanızın veya www.traceper.com adresindeki "Arkadaşlık İstekleri" menüsünden bu arkadaşlık isteğini kabul ya da reddedebilirsiniz.',
	'new' => 'yeni',
	' totally ' => ' toplamda ',
	' friendship request(s) you have' => ' arkadaşlık isteğiniz var',
	'Type a friend\'s name' => 'Arkadaşınızın adını girin',
	'You have no pending friendship requests at the moment.' => 'Şu an için bekleyen arkadaşlık isteğiniz bulunmuyor.',
	'This user does not share his/her location info at the moment' => 'Bu kullanıcı şu an için konum bilgilerini paylaşmıyor'
	
	
	
								
);


?>