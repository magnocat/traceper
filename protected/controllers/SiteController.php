...
// 	}
				...
				{					
					...
					else
					{
						if (isset($_REQUEST['client']) && $_REQUEST['client']=='mobile')
						{
							...
							$this->sendErrorMail('Error in actionLogin()', $errorMessage);							
						}
						else {
							//echo 'Model NOT valid in SiteController';
	...
	public function actionContinueLogin() {
		...
		if(isset($_REQUEST['LoginForm']['email']) && isset($_REQUEST['LoginForm']['password']))
		{
			...
			if (isset($_REQUEST['client']) && $_REQUEST['client']=='mobile')
			{
				...
				else
				{
					...
					$this->sendErrorMail('MOBILE Error in actionContinueLogin()', $errorMessage);					
					Yii::app()->end();
				}
			}		
		...
		else
		{
			...
			$errorMessage = "Missing parameter";
			$this->sendErrorMail('Error in actionContinueLogin()', $errorMessage);			
			Yii::app()->end();			
		}						
	}
		...
		if (isset($_REQUEST['client']) && $_REQUEST['client']=='mobile')
		{
					...
					isset($_REQUEST['language']) && isset($_REQUEST['deviceId']) && isset($_REQUEST['androidVer']) && isset($_REQUEST['appVer']))
			{
				...
				if($password != $autoGeneratedPassword)
				{
					...
					$errorMessage = "Mobile-generated password and web-generated password do not match!";
					$this->sendErrorMail('MOBILE Error in actionFacebookLogin()', $errorMessage);					
					Yii::app()->end();
				}
		...
		if (isset($_REQUEST['client']) && $_REQUEST['client']=='mobile')
		{
				...
				isset($_REQUEST['language']) && isset($_REQUEST['deviceId']) && isset($_REQUEST['androidVer']) && isset($_REQUEST['appVer']))
			{
				...
				if($password != $autoGeneratedPassword)
				{
					...
					$errorMessage = "Mobile-generated password and web-generated password do not match!";
					$this->sendErrorMail('MOBILE Error in actionContinueFacebookLogin()', $errorMessage);					
					Yii::app()->end();					
				}
...
// 		Fb::warn("name: $name", "actionContinueFacebookLogin()");		
		...
		else if (Users::model()->saveFacebookUser($fbEmail, md5($autoGeneratedPassword), $name, $fbId, 1/*account_type*/, $registrationMedium, $language)) 
		{
			...
			$errorMessage = "User could not be registered!";
			$this->sendErrorMail('Error in actionContinueFacebookLogin()', $errorMessage);			
			Yii::app()->end();
		}
	}
		...
		if (isset($_REQUEST['client']) && $_REQUEST['client']=='mobile')
		{
					...
					isset($_REQUEST['language']) && isset($_REQUEST['deviceId']) && isset($_REQUEST['androidVer']) && isset($_REQUEST['appVer']))
			{
				...
				if($password != $autoGeneratedPassword)
				{
					...
					$errorMessage = "Mobile-generated password and web-generated password do not match!";
					$this->sendErrorMail('MOBILE Error in actionSwitchToFacebookLoginPermanently()', $errorMessage);					
					Yii::app()->end();
				}
...
// 		Fb::warn("name: $name", "actionSwitchToFacebookLoginPermanently()");
		...
		if (Users::model()->updateTraceperUserAsFacebookUser($fbEmail, md5($autoGeneratedPassword), $name, $fbId, $language))
		{
			...
			$this->sendErrorMail('Error in actionSwitchToFacebookLoginPermanently()', $errorMessage);			
			Yii::app()->end();
		}
	}	
	...
	private function commonLogin($email, $password, $language, $directLogin = false)
	{
		...
		else
		{
			...
			$this->sendErrorMail('Error in commonLogin()', $errorMessage);			
			Yii::app()->end();
		}
	}	
		...
		if(isset($_POST['ChangePasswordForm']))
		{
			...
			if($model->validate()) {
				...
				else
				{
					...
					$this->sendErrorMail('Error in actionChangePassword()', $errorMessage);					
				}
...
// 					Yii::app()->end();
				...
				{				
					...
					$this->sendErrorMail('Error in actionResetPassword()', $errorMessage);					
				}
			...
			if($model->validate()) {
				...
				else
				{
					if (isset($_REQUEST['client']) && $_REQUEST['client']=='mobile')
					{
						...
						$this->sendErrorMail('MOBILE Error in actionRegister()', $errorMessage);						
					}
					else
					{
						...
						$errorMessage = "Error in saving user candidate!";
						$this->sendErrorMail('Error in actionRegister()', $errorMessage);						
					}
...
//		Fb::warn(Yii::app()->session['usersPageSize'], "usersPageSize");
	...
	{	
		...
		{
			$this->sendErrorMail($title, $message);		
