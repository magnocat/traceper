...
// 	}
	...
	public function actionContinueLogin() {
		...
		if(isset($_REQUEST['LoginForm']['email']) && isset($_REQUEST['LoginForm']['password']))
		{
			...
			$this->commonLogin($_REQUEST['LoginForm']['email'], $_REQUEST['LoginForm']['password'], $language, false/*$directLogin*/, "actionContinueLogin()");
		}
		else
		{
	...
	public function actionFacebookLogin()
	{
		...
		if (isset($_REQUEST['client']) && $_REQUEST['client']=='mobile')
		{
			if ((isset($_REQUEST['email']) && ($_REQUEST['email'] != NULL)) && 
				(isset($_REQUEST['fbId']) && ($_REQUEST['fbId'] != NULL)) &&
				(isset($_REQUEST['name']) && ($_REQUEST['name'] != NULL)) && 
				(isset($_REQUEST['password']) && ($_REQUEST['password'] != NULL)) &&
				(isset($_REQUEST['language']) && ($_REQUEST['language'] != NULL)) && 
				(isset($_REQUEST['deviceId']) && ($_REQUEST['deviceId'] != NULL)) && 
				(isset($_REQUEST['androidVer']) && ($_REQUEST['androidVer'] != NULL)) && 
				 isset($_REQUEST['appVer']) && ($_REQUEST['appVer'] != NULL))
			{
				$fbId = $_REQUEST['fbId'];
			...
			else
			{
				...
				$errorMessage = '"Missing Parameter" error occured:'.'<br/><br/>';
					
				if(isset($_REQUEST['email']) == false)
				{
					$errorMessage .= '"email" is missing!'.'<br/>';
				}
				else if($_REQUEST['email'] == NULL)
				{
					$errorMessage .= '"email" is  NULL!'.'<br/>';
				}

				if(isset($_REQUEST['fbId']) == false)
				{
					$errorMessage .= '"fbId" is missing!'.'<br/>';
				}
				else if($_REQUEST['fbId'] == NULL)
				{
					$errorMessage .= '"fbId" is  NULL!'.'<br/>';
				}
				...
				else if($_REQUEST['name'] == NULL)
				{
					$errorMessage .= '"name" is  NULL!'.'<br/>';
				}
				...
				{
					$errorMessage .= '"password" is  NULL!'.'<br/>';
				}

				if(isset($_REQUEST['language']) == false)
				{
					$errorMessage .= '"language" is missing!'.'<br/>';
				}
				else if($_REQUEST['language'] == NULL)
				{
					$errorMessage .= '"language" is  NULL!'.'<br/>';
				}

				if(isset($_REQUEST['deviceId']) == false)
				{
					$errorMessage .= '"deviceId" is missing!'.'<br/>';
				}
				else if($_REQUEST['deviceId'] == NULL)
				{
					$errorMessage .= '"deviceId" is  NULL!'.'<br/>';
				}

				if(isset($_REQUEST['androidVer']) == false)
				{
					$errorMessage .= '"androidVer" is missing!'.'<br/>';
				}
				else if($_REQUEST['androidVer'] == NULL)
				{
					$errorMessage .= '"androidVer" is  NULL!'.'<br/>';
				}

				if(isset($_REQUEST['appVer']) == false)
				{
					$errorMessage .= '"appVer" is missing!'.'<br/>';
				}
				else if($_REQUEST['appVer'] == NULL)
				{
					$errorMessage .= '"appVer" is  NULL!'.'<br/>';
				}

				$errorMessage .= '<br/>';
				$errorMessage .= "fbEmail: ".$_REQUEST['email'].'<br/>';
				$errorMessage .= "fbId: ".$_REQUEST['fbId'].'<br/>';				
		...
		$this->commonLogin($fbEmail, $autoGeneratedPassword, $language, $directLogin, "actionFacebookLogin()");
	}
...
// 		Fb::warn("name: $name", "actionContinueFacebookLogin()");		
		...
		else if(Users::model()->isFacebookUserRegistered($fbEmail, $fbId) == true)
		{
			...
			$this->commonLogin($fbEmail, $autoGeneratedPassword, $language, $directLogin, "actionContinueFacebookLogin()");
		}
		else if (Users::model()->saveFacebookUser($fbEmail, md5($autoGeneratedPassword), $name, $fbId, AccountType::FacebookUser, $registrationMedium, $language)) 
		{
			Fb::warn("saveFacebookUser() successful", "actionContinueFacebookLogin()");			
			Users::model()->setTermsAccepted($fbEmail);
			
			//Kucuk bir ihtimal de olsa bu kullanici eger daha once normal kayit islemini baslatip yarida birakmis biri ise (yani hala UserCandidates tablosunda bulunuyor ise)
			//o tablodan sil
			if(UserCandidates::model()->isUserCandidate($fbEmail) == true)
			{
				UserCandidates::model()->deleteCandidate($fbEmail);
			}
			
			//Kullanici zaten simdi kaydedildigi icin direk login olabilir, ek kontrole gerek yok
			$this->commonLogin($fbEmail, $autoGeneratedPassword, $language, true/*$directLogin*/, "actionContinueFacebookLogin()");
		}
		else {
	...
	public function actionOldFacebookUserLogin()
	{
		...
		if(isset($_POST['LoginForm']))
		{
			...
			if($model->validate()) {
				$this->commonLogin($fbEmail, $model->password, $language, false/*$directLogin*/, "actionOldFacebookUserLogin()");
			}
			else
			{
				if (Yii::app()->request->isAjaxRequest)
...
// 		Fb::warn("name: $name", "actionSwitchToFacebookLoginPermanently()");
		...
		if (Users::model()->updateTraceperUserAsFacebookUser($fbEmail, md5($autoGeneratedPassword), $name, $fbId, $language))
		{
			...
			$this->commonLogin($fbEmail, $autoGeneratedPassword, $language, true/*$directLogin*/, "actionSwitchToFacebookLoginPermanently()");
		}
		else {
	...
	private function commonLogin($email, $password, $language, $directLogin = false, $calledFrom = "unknown")
	{
		...
		else
		{
			...
			if($directLogin)
			{
				$errorMessage = "Unknown login error occured during DIRECT login!"."<br/><br/>";
				$errorMessage .= "email:".$email."<br/>";
				$errorMessage .= "password:".$password."<br/>";
			}
			else
			{
				$errorMessage = "Unknown login error occured during login (NOT direct)!"."<br/><br/>";
				$errorMessage .= "email:".$email."<br/>";
				$errorMessage .= "password:".$password."<br/>";				
			}
			...
			if($userCandidate != null)
			{
				...
				if ($generatedKey == $key)
				{
					...
					catch (Exception $e)
					{
						...
						{
							$result = Yii::t('site', 'Sorry, an error occured during activating your account. If the error persists, please contact us.');

							$errorMessage = "An exception occured during activating a user account.".'</br></br>';
							$errorMessage .= "Exception code: ".$e->getCode().'</br></br>';
							$errorMessage .= "Exception message: ".$e->getMessage();
							$this->sendErrorMail('accountActivateException', 'Error in actionActivate()', $errorMessage);
						}									
					}					
				}
				else 
